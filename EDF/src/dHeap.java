...
/**
 * Title: class dHeap Description: implements dHeapInterface
 * 
 * @author Ruoxin Huang
 * @version 1.0
 * @since 02-19-2016
...
class dHeap<T extends Comparable<? super T>> implements dHeapInterface<T> {
	...
	/**
	 * The default constructor of binary heap
	 * 
	 * @param heapSize
	 *            an initial capacity
	 */
	...
	/**
	 * Removes the smallest element in the heap cannot return from empty heap.
	 * 
	...
	public T removeSmallest() throws NoSuchElementException {
		if (nelems == 0) // If remove from empty set throw
							// NoSuchElementException
		{
			throw new NoSuchElementException();
		}
		T data = array[0]; // Store the data on the element to be removed
		array[0] = array[nelems - 1]; // remove top element and put the last
										// element to the top
	...
	private void bubbleUp(int indx) {
		...
		if (array[indx].compareTo(array[parent]) == 1) // If child greater than
														// parent, then exit
		{
			return;
		}
	 ...
	 * 
	 * @param indx
	 *            the index of the element to trickle down
	 */
	private void trickleDown(int indx) {
		int smallestChild = smallestChild(indx); // The index of the smallest
													// child
		if (smallestChild >= nelems - 1) // If already at bottom, then exit
		{
			return;
		}
		if (array[indx].compareTo(array[smallestChild]) == -1) // If parent
																// smaller than
																// child, then
																// exit
		{
	...
	private int smallestChild(int indx) {
		int smallestIndx = indx * children + 1; // Index of smallest children
												// set to first child
		for (int i = 2; i <= children; i++) // Loop through every child
		{
			if (indx * children + i >= nelems - 1) // If no more children to
													// compare, return smallest
													// chiild
			{
				return smallestIndx;
			}
	...
	/**
	 * Helper method used to double the size of the heap when full creates new
	 * heap with double size plus 1 to counter empty heaps
	 * 
	 */
	public void doubleSize() {
		T[] doubleSize = (T[]) new Comparable[size * 2 + 1]; // Make new array
																// with double
																// the size plus
																// 1
