...
(function() {
	...
	g.ret = g.prototype = {
		...
		html : function(t) {
			if (t || t == '') {
				this[0].innerHTML = t;return this
		...
		val : function(v) {
			if (v || v == '') {
				this[0].value = v;return this
			} else return this[0].value
		},
		acls : function(c, p) {
			this[0].className = p ? this[0].className + ' ' + c : c;return this
		},
		rcls : function() {
			var a = g.ie ? 'className' : 'class';
			this[0].removeAttribute(a, 0);return this
		},
		crte : function(e) {
		...
		attr : function(k, v) {
			...
			if (v) {
				this[0].setAttribute(k, v, 0);return this
			} else {
		...
		ratt : function(n) {
			...
			this[0].removeAttribute(n, 0);return this
		},
		aevt : function(n, f) {
		...
		alnk : function(c) {
			if (g.ie) return this[0].createStyleSheet(c).owningElement;else {
				...
				e.rel = 'stylesheet';
				e.type = 'text/css';
				e.href = c;this[0].getElementsByTagName('head')[0].appendChild(e);return e
			}
		}
	};
	...
	g.exend = g.ret.exend = function() {
			...
			if ((options = arguments[i]) != null)
				for (var b in options) {
					...
					if (deep && copy && typeof copy == 'object' && !copy.nodeType)
						a[b] = g.extend(deep, c || (copy.length != null ? [] : {}), copy);
		...
		click : function(f) {
			this[0].onclick = f;return this
		},
		blur : function(f) {
			this[0].onblur = f;return this
		},
		focus : function(f) {
			if (f)
				this[0].onfocus = f;else this[0].focus();
			return this
		},
		msdown : function(f) {
			this[0].onmousedown = f;return this
		},
		msmove : function(f) {
			this[0].onmousemove = f;return this
		},
		msover : function(f) {
			this[0].onmouseover = f;return this
		},
		msout : function(f) {
			this[0].onmouseout = f;return this
		},
		msup : function(f) {
			this[0].onmouseup = f;return this
		},
		submit : function(f) {
			if (f)
				this[0].onsubmit = f;else this[0].onsubmit();
			return this
		},
		cmenu : function(f) {
			this[0].oncontextmenu = f;return this
		},
		hover : function(r, t) {
		...
		path : function(t) {
			...
			if (!g.ie || (j.match(/msie (\d+)/) || [])[1] == 8 && !/opera/.test(j)) return a;else {
				...
				if (g.inde(a, '../') != -1) {
					...
					a = document.location.protocol + '//' + document.location.host + a;return a
				} else return b + '/' + a
			}
		},
		idtd : function(d) {
		...
		vsiz : function(a) {
			if (g.ie) {
				var b,
					doc = a.document.documentElement;
		...
		nosel : function(o) {
			if (g.ie) {
				...
				if (g.mz)
					...
					o.style.KhtmlUserSelect = 'none';else
		...
		gtbyn : function(n, v, w) {
			var a,
				co = (w || window).document.getElementsByName(n);
			if (!v) return co;else {
	...
	g.exend({
		...
		cleanup : function() {
			if (window._lhgcleanobj)
				this.citem = window._lhgcleanobj.citem;else {
				this.citem = [];
				window._lhgcleanobj = this;J(window).addentex('unload', this.lhg_clean)
			}
		}
	...
	J.exend({
		panel : function(b, w) {
			...
			if (b) {
				while (this._win.parent && this._win.parent != this._win) {
					...
					this._win = this._win.parent;r_win.push(this._win)
				}
			}
			if (w) {
				for (var i = 0; i < w.length; i++) r_win.push(w[i])
			}
			a = this._ifrm = J(this._win.document).crte('iframe');J(a).attr({
			...
			doc.open();doc.write('<html><head>' + c + '<\/head><body style="margin:0px;padding:0px;"><\/body><\/html>');doc.close();
		...
		show : function(x, y, e, w, h) {
			var a = this._main,
				iw,
				ih;
			J(this._ifrm).stcs('display', 'block');J(a).stcs({
				width : w ? w + 'px' : '',
				height : h ? h + 'px' : ''
			});
			iw = a.offsetWidth;
		...
		send : function(u, m, p, f, x) {
			...
			if (a) {
				b.onreadystatechange = function() {
					if (b.readyState == 4) {
						ret = (x == 0) ? b.responseText : b.responseXML;f(ret);
						//delete (b);
						return
					} else return false
				}
